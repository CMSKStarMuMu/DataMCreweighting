...
Save_Dir=''
if (year == 2016):
    Save_Dir = './model/2016/3class/2016_XGBV5_eta10_subsample10_depth7_round1000.json'
elif (year==2017) : 
...
pr_phsp=np.array(trained_bst.predict(xg_phsp_only,validate_features=False).reshape(phsp_only_Y.shape[0], 3))

#weight_test=pr_test[:,1]/pr_test[:,0]
weight_phsp=(pr_phsp[:,1]-pr_phsp[:,2])/pr_phsp[:,0]
weight_phsp[weight_phsp<0] = 0
...
MCwFile = r.TFile("./files/2016/JPsiK_reweight_XGBV7_{}_3class_reset0.root".format(year),"RECREATE")
